...
from config.settings import PERCENTAGE_BALANCE_FOR_SWAP


async def GetDataForSwap(client: Client, swap_to_eth = False) -> dict:
    try:
        ...
        from_token_data = ContractInfo.GetData(ETH_ADDRESS)
...
async def GetDataForLP(client: Client, dex_name: str) -> dict:
    try:
        ETH_ADDRESS = ContractInfo.ETH.get('address')
        USDT_ADDRESS = ContractInfo.USDT.get('address')
        USDC_ADDRESS = ContractInfo.USDC.get('address')
        ...
        if (token_one_address == ETH_ADDRESS and token_two_address == USDT_ADDRESS) or (token_one_address == USDT_ADDRESS and token_two_address == ETH_ADDRESS):
            ...
            if dex_name == 'jediswap':
                pooled_token_data = ContractInfo.GetData(
            ...
            if dex_name == 'jediswap':
                pooled_token_data = ContractInfo.get_data_from_contract(
